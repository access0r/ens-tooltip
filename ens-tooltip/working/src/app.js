  ...
  if (matches) {
    return text.split(ensNameRegex).reduce((prev, current, i) => {
      if (!i) {
        ...
        return [...prev, <span key={i} className="ens-name">{matches[i - 1]}</span>, current];
      }
    }, []);
  } else {
    return text;
...
const sendTransaction = async () => {
  ...
  const transaction = {
    ...
    value: web3.utils.toWei('0.1', 'ether'),  // Replace with the amount of Ether to send
...
const MyComponent = () => {
  ...
  return (
    <div onClick={onClick}>
      ...
      {/* Render the tooltip */}
      {tooltipData && (
        <div className="tooltip" style={{ ...config.tooltip, left: tooltipPosition.x, top: tooltipPosition.y }}>
          {/* Render the avatar */}
          <img src={tooltipData.avatar} alt="avatar" className="avatar-img" />

          {Object.keys(tooltipData).map((key) => {
            if (key !== 'avatar' && config.displayOptions['show' + key.charAt(0).toUpperCase() + key.slice(1)]) {
              return (
                <div key={key} className="nameRecord" style={config.nameRecord}>
                  <div className="dataTitle" id={key} style={config.dataTitle}>{key}</div>
          ...
          <div className="transactions">
            {/* ... code for rendering transactions ... */}
          </div>

          {/* Render the send transaction button */}
